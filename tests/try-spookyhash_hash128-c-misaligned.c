...
main (int argc, char *argv[])
{
  ...
  const size_t misalignment_max = 16 * sizeof (uint64_t);

  /* Try various misalignments. */
  for (size_t i = 0; i != misalignment_max; i += 1)
    {
      uint64_t hash1;
      uint64_t hash2;
      uint8_t *message = malloc (i + length);
      fill_message (message + i, length, pattern);
      spookyhash_hash128 (message + i, length, seed1, seed2,
                          &hash1, &hash2);
      free (message);
